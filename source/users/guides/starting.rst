...
.. note::
    En auto-hébergement rendez-vous sur `la page de connection <https://127.0.0.1/raspisms>`_, les identifiants ont été générés lors de l'installation et sont disponible dans le fichier ``/usr/share/raspisms/.credentials``.

2. Créez un premier téléphone
==============================

Afin de pouvoir recevoir et envoyer des SMS depuis l'application vous devez dans un premier temps créer un téléphone qui sera utilisé pour envoyer et recevoir des messages. Pour cela vous pouvez utiliser différents fournisseurs de services (:ref:`voir Les Adaptateurs <adapters>`), mais nous vous recommandons l'utilisation :ref:`d'OVH avec un numéro virtuel <users_adapters_ovh_virtual_number>` qui à notre sens fourni le service le plus accessible et éfficace.
...
Dans le champs **"Numéros cibles"**, renseignez le numéro auquel vous souhaitez envoyer le SMS. Il est possible d'ajouter d'autres numéros en cliquant sur le "+" en dessous du champs.

Enfin, dans le champs **"Numéro à employer"**, séléctionnez le nom du téléphone que vous venez de créer. Si vous laisser le champs à sa valeure par défaut "N'importe lequel", un téléphone sera choisi au hasard parmis ceux que vous avez créé (avec un seul téléphone cela n'as donc pas d'importance).

Il ne vous reste plus qu'à cliquez sur le bouton **"Enregistrer le SMS"**.

4. Vérifier le status du SMS
============================

Vous pouvez maintenant vérifier si le SMS a bien été envoyés, et éventuellement reçu, en vous rendant dans la partie **"SMS"**, **"SMS Envoyés"**.
